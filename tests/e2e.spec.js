...
test.describe("table-bar-chart component", () => {
  ...
  test("renders grouped bars for multiple series", async ({ page }) => {
    await page.goto("/tests/fixtures/grouped.html");
    await expect(page.locator("h1")).toHaveText("Grouped Sales");
    // Expect each row to render 2 bars (Online, Retail)
    const groupedBars = page.locator(".grouped-bars .bar");
    await expect(groupedBars).toHaveCount(6);
  });

  test("renders stacked bars when stacked attribute present", async ({
    page,
  }) => {
    await page.goto("/tests/fixtures/stacked.html");
    await expect(page.locator("h1")).toHaveText("Stacked Revenue");
    // Outer stacked bars (fieldsets with class bar-group.stack)
    const outer = page.locator(".bar-group.stack");
    await expect(outer).toHaveCount(3);
    // Each outer has segment buttons inside (buttons with class bar.segment)
